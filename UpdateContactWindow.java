...
public class UpdateContactWindow extends JFrame{
			...
			public void actionPerformed(ActionEvent evt){				
				if(!hasEmptyField()){
					...
					if(index != -1){
						...
						isUpdated = contactsList.setContact(index,contact);
						...
						JOptionPane.showMessageDialog(null,"Conatct \""+txtSearch.getText()+"\" does not exist!","Error",JOptionPane.ERROR_MESSAGE);
					}
				}
			}
		});
		...
		for(int i=0; i < textFieldList.length; i++){
			if(textFieldList[i] != null){
				...
				switch(textFieldList[i].getName()){
					case "contactIdTextField" : {
						if(text.isEmpty()){
							JOptionPane.showMessageDialog(null,"Contact has not been loaded to update!","Warning!",JOptionPane.WARNING_MESSAGE);
							return true;
						}
					}
					break;
	...
	public void loadContactDetails(){
		...
		ContactsList contactsList = getContactsList();
		int index = contactsList.searchByNameOrContactNo(new Contact(null,searchText,searchText,null,null,null));
		
		if(index != -1){
			Contact contact = contactsList.getContact(index);
			...
			JOptionPane.showMessageDialog(null,"Contact \""+searchText+"\" could not be found!","",JOptionPane.INFORMATION_MESSAGE);
		}
	}
