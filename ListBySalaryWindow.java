...
public class ListBySalaryWindow extends JFrame{
	private JButton btnBackToHomePage;	
	
	private DefaultTableModel defaultTableModel;
	private JTable contactsTable;
	
	public ListBySalaryWindow(){
		setSize(1100,750);
		setLocationRelativeTo(null);
		setDefaultCloseOperation(DISPOSE_ON_CLOSE);
		setResizable(false);
		
		JLabel lblListByName = new JLabel("LIST CONTACTS BY SALARY",SwingConstants.CENTER);
	...
	private double getDoubleValue(Contact contact){
		String numericalString = contact.getSalary().replace(",","");
		double doubleValue = Double.parseDouble(numericalString);
		return doubleValue;
	}
