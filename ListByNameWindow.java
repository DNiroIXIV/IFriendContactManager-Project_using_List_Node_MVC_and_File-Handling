...
public class ListByNameWindow extends JFrame{
	...
	public ListByNameWindow(){
		...
		Contact[] contactDbListedByName = listContactsByName();
		
		for (int i = 0; i < contactDbListedByName.length; i++){
			Contact contact = contactDbListedByName[i];
	...
	public static Contact[] listContactsByName(){
		...
		Contact[] tempContactsArray = contactsList.getAllContacts();
		
		for (int i = tempContactsArray.length-1; i > 0; i--){
			for (int j = 0; j < i; j++){
				int valueOfCompareToIgnoreCase = tempContactsArray[j].nameCompareToIgnoreCase(tempContactsArray[j+1]);
				if(valueOfCompareToIgnoreCase > 0){
					swapContacts(tempContactsArray, j);
				}else if(valueOfCompareToIgnoreCase == 0){
					int valueOfCompareTo = tempContactsArray[j].nameCompareTo(tempContactsArray[j+1]);
	...
	private static void swapContacts(Contact[] tempContactsArray, int index){
